...
export const __DO_MANIFEST__ = [
  ...
  // Tangent live race + matchmaking (must also be in wrangler.toml bindings
  // AND migrations[].new_sqlite_classes, or they never deploy).
  { binding: 'GAME_ROOMS', className: 'AppGameRoom', sqlite: true },
  { binding: 'MATCHMAKER_ROOMS', className: 'AppMatchmakerRoom', sqlite: true },
...
/**
 * AppJobRoom — durable background-job queue (DO binding kept for forward-compat;
 * removing it would need a storage migration). v1 enqueues no jobs, so onJob is
 * an intentional no-op. Wire real background work here when a feature needs it.
 */
export class AppJobRoom extends JobRoom<Env> {
  ...
  protected async onJob(_job: Job, _ctx: JobContext): Promise<unknown> {
    return undefined
  }
}

/**
 * AppGameRoom — the live race room. One DO per race, addressed by roomId via
 * `idFromName(roomId)` (quick:<uuid>, ranked:<uuid>, chaos:<uuid>,
 * room:<code>). Authoritative state lives in `gameState`; the heavy logic
 * (move gate, first-arrival adjudication, ghosts, chaos netcode, onGameEnd
 * run-writes) lives in `src/game/app-game-room.ts` and `src/game/engine.ts`.
 */
export class AppGameRoom extends TangentGameRoom {}

/**
 * AppMatchmakerRoom — per-mode matchmaking allocator (mm:quick / mm:chaos /
 * mm:ranked), the single-threaded serialization point. Logic lives in
 * `src/game/app-matchmaker-room.ts`.
 */
export class AppMatchmakerRoom extends TangentMatchmakerRoom {}
...
export interface Env extends DOBindings<typeof __DO_MANIFEST__> {
  ...
  /**
   * Global/shared article render cache (KV). Key `art:v1:<canonicalTitle>` ->
   * the processed artifact {pageId, canonicalTitle, servedHtml, allowedTitles}.
   * Provisioned via `id="auto"` in wrangler.toml. Read by the article pipeline.
   */
  ARTICLE_CACHE: KVNamespace
  /**
   * R2 bucket for generated share cards (PNG, written by the generate-card
   * job). Provisioned via `bucket_name="auto"` in wrangler.toml.
   */
  CARDS: R2Bucket
  /**
...
// ---------------------------------------------------------------------------
// Article route — serve a rendered, link-classified Wikipedia article.
//
// Public (anon ok — guest-first). Returns ONLY the served HTML + the reach ids.
// The allowed-move title set stays server-side (the move gate reads it); never
// shipping it to the client is the core anti-cheat seam (FINAL-SPEC §3).
// ---------------------------------------------------------------------------
...
app.get('/api/article', async (c) => {
  ...
  try {
    const article = await getArticle(c.env, title)
    return c.json({
      servedHtml: article.servedHtml,
      pageId: article.pageId,
      canonicalTitle: article.canonicalTitle,
    })
  } catch {
    return c.json({ error: 'Article load failed' }, 502)
  }
})
...
app.get('/ws/yjs/:docId', async (c) => {
  ...
  const doUrl = new URL(c.req.url)
  // Scrub ALL client-supplied identity (userName/userEmail/userImageUrl too, not
  // just token/userId) before re-asserting from the verified JWT, so a client
  // cannot spoof its display identity into the DO.
  scrubIdentity(doUrl)
  doUrl.searchParams.set('userId', auth.userId)
...
// ---------------------------------------------------------------------------
// Live race + matchmaking WebSocket routes (Tangent custom).
//
// FIX for the SDK anon-role footgun (docs/founder/sdk-issues.md S-1):
// ROLE_ANONYMOUS = 'viewer' and the stock `wsRoute` applies a role only for
// authenticated users, so an anonymous GameRoom socket lands as
// viewer/canWrite:false and `sendInput()` no-ops — which breaks the locked
// guest-first requirement ("race with no signup"). This inline handler grants
// role 'member' to BOTH authenticated and anonymous PLAYER connections.
//
// Identity is never trusted from the client beyond an opaque guestId: all
// client-supplied identity params are scrubbed, then userId is re-applied from
// the verified JWT (authed) or a stable `anon:<guestId>` (guest, from a
// cookie/query) so reconnect/rejoin resolves to the same player. The guestId
// can only set the player's OWN identity, never elevate privilege.
//
// Matchmaker connections reuse this route via the `mm:` roomId prefix and are
// dispatched to the MATCHMAKER_ROOMS namespace.
// ---------------------------------------------------------------------------

const GUEST_ID_RE = /[^A-Za-z0-9_-]/g
...
function readGuestId(c: any): string | null {
  const url = new URL(c.req.url)
  const fromQuery = url.searchParams.get('guestId')
  const fromCookie = readCookie(c.req.header('Cookie') ?? '', 'tg_guest')
  const raw = (fromQuery ?? fromCookie ?? '').replace(GUEST_ID_RE, '').slice(0, 64)
  return raw || null
}

function scrubIdentity(url: URL): void {
...
app.get('/ws/game/:roomId', async (c) => {
  ...
  const doUrl = new URL(c.req.url)
  scrubIdentity(doUrl)
  // PLAYER connections are always members (canWrite:true), authed or guest.
  ...
  if (auth) {
    ...
    if (guestId) doUrl.searchParams.set('userId', `anon:${guestId}`)
  }

  // mm:* roomIds are matchmaker queues; everything else is a live race room.
  const ns = roomId.startsWith('mm:') ? c.env.MATCHMAKER_ROOMS : c.env.GAME_ROOMS
  const stub = ns.get(ns.idFromName(roomId))
  return stub.fetch(new Request(doUrl.toString(), c.req.raw))
})

app.get('/ws/game-watch/:roomId', async (c) => {
  const roomId = c.req.param('roomId')
  const url = new URL(c.req.url)
  const token = url.searchParams.get('token')
  ...
  const doUrl = new URL(c.req.url)
  scrubIdentity(doUrl)
  // Spectators are FORCED to viewer (canWrite:false) regardless of auth — the
  // safe self-downgrade direction. sendInput/start/end no-op at the SDK layer.
  doUrl.searchParams.set('role', 'viewer')
  if (auth) {
    doUrl.searchParams.set('userId', auth.userId)
    if (auth.claims.name) doUrl.searchParams.set('userName', auth.claims.name)
  }
...
// Resolve a guest id for an anonymous action caller (body param or cookie),
// scrubbed to an opaque token. Guest-first: the async race + matchmaking
// actions run with no signup, identity = `anon:<guestId>`.
function resolveActionGuestId(c: any, params: Record<string, unknown>): string | null {
  const fromParam = typeof params.guestId === 'string' ? params.guestId : ''
  const fromCookie = readCookie(c.req.header('Cookie') ?? '', 'tg_guest') ?? ''
  const raw = (fromParam || fromCookie).replace(GUEST_ID_RE, '').slice(0, 64)
  return raw || null
}
...
app.post('/api/actions/:name', async (c) => {
  ...
  // Identity is the verified JWT subject, or a stable `anon:<guestId>` for a
  // signed-out caller. Anything else is unauthorized. The action's tools always
  // run with X-App-Action (RBAC bypass); `userId` is only ever the caller's
  // OWN subject, never an elevation.
  const auth = await resolveAuth(c.req.raw, c.env)
  let subjectId: string
  let callerJwt = ''
  if (auth) {
    subjectId = auth.userId
    callerJwt = c.req.header('Authorization')?.slice(7) ?? ''
  } else {
    const guestId = resolveActionGuestId(c, params)
    if (!guestId) return c.json({ error: 'Unauthorized' }, 401)
    subjectId = `anon:${guestId}`
  }

  const tools = createActionTools(c.env, subjectId, callerJwt)
  const result = await action({ userId: subjectId, params, tools, env: c.env, callerJwt })
...
// ---------------------------------------------------------------------------
// Open Graph / Twitter card middleware + share-card render target.
//
// Intercepts the shareable deep links and injects OG/Twitter meta into the SPA
// shell so unfurls (iMessage, Slack, Discord, X) show a real preview. These
// prefixes are added to wrangler.toml `run_worker_first` so they reach the
// worker BEFORE the SPA asset fallback (paths outside that set are served
// straight from assets and never hit the worker). Dynamic values are STUBBED
// for now; later waves read the real run/room/challenge/daily data.
//
// Route table (RESOLUTIONS B2): /i/:code invite-join · /r/:runId replay ·
// /c/:id challenge · /today daily · /f/:code friend-add · /_card/:runId card.
// ---------------------------------------------------------------------------
...
function renderMetaTags(m: OgMeta): string {
  const tags = [
    `<meta property="og:type" content="website" />`,
    `<meta property="og:site_name" content="Tangent" />`,
    `<meta property="og:title" content="${escMeta(m.title)}" />`,
    `<meta property="og:description" content="${escMeta(m.description)}" />`,
    `<meta property="og:url" content="${escMeta(m.url)}" />`,
    `<meta name="twitter:card" content="${m.image ? 'summary_large_image' : 'summary'}" />`,
    `<meta name="twitter:title" content="${escMeta(m.title)}" />`,
    `<meta name="twitter:description" content="${escMeta(m.description)}" />`,
  ]
  if (m.image) {
    tags.push(`<meta property="og:image" content="${escMeta(m.image)}" />`)
    tags.push(`<meta name="twitter:image" content="${escMeta(m.image)}" />`)
  }
  return tags.join('\n    ')
}

async function serveShellWithMeta(c: any, meta: OgMeta): Promise<Response> {
  const url = new URL(c.req.url)
  url.pathname = '/index.html'
  const res = await c.env.ASSETS.fetch(new Request(url.toString(), c.req.raw))
  if (!res.ok) return res
  const html = await res.text()
  const injected = html.replace('</head>', `    ${renderMetaTags(meta)}\n  </head>`)
...
const ogRoute = (build: (c: any) => OgMeta) => (c: any) => serveShellWithMeta(c, build(c))

app.get(
  '/today',
  ogRoute((c) => ({
    title: "Today's Tangent",
    description: 'Everyone gets the same line. Fewest clicks wins. Find the line.',
...
app.get(
  '/i/:code',
  ogRoute((c) => ({
    title: 'Race me on Tangent',
    description: `Join room ${c.req.param('code')} and find the line first. No signup.`,
    url: new URL(c.req.url).toString(),
  })),
)
...
app.get(
  ...
  ogRoute((c) => ({
    title: 'A Tangent run',
    description: 'Watch this path between two Wikipedia articles, then beat it.',
    url: new URL(c.req.url).toString(),
  })),
)
...
app.get(
  '/c/:id',
  ogRoute((c) => ({
    title: 'Beat my Tangent',
    description: 'Someone challenged you to a shorter line. No signup to play.',
    url: new URL(c.req.url).toString(),
  })),
)
...
app.get(
  '/f/:code',
  ogRoute((c) => ({
    title: 'Add me on Tangent',
    description: 'Accept this friend invite and race together.',
    url: new URL(c.req.url).toString(),
  })),
)
