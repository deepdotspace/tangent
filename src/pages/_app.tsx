...
/**
 * Tangent is a full-bleed, brand-chromed product: every screen carries its own
 * top bar / radial stage, so the scaffold's generic <Navigation /> is dropped
 * here. The provider stack (toasts -> auth -> record/scope) is preserved.
 */
    ...
    <ToastProvider>
      <DeepSpaceAuthProvider>
        <AuthBoot>
          ...
          <div data-testid="app-root" style={{ minHeight: '100vh', background: 'var(--bg)' }}>
            <Suspense fallback={<div style={{ display: 'grid', placeItems: 'center', minHeight: '100vh', color: 'var(--mute)' }}>Loading...</div>}>
              <Outlet />
            </Suspense>
          </div>
        </AuthBoot>
